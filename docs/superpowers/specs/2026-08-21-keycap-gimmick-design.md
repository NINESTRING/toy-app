...
**배경은 화면을 가득 채운다.** 기믹이 콘텐츠 영역 전체를 받고 셸 크롬(햅틱 안내 ·
축 칩 · 카운트)이 `pointerEvents="box-none"` 오버레이로 그 위에 뜬다. 기믹마다
플래그를 두는 대신 모든 기믹에 같은 모델을 적용한다 — 클리커·다이얼은 컨테이너에
`flex: 1`만 붙으면 보이는 결과가 같다.

캔버스는 `onSize`로 실제 크기를 받고, 키캡 도형은 **300pt 설계 좌표계**에 적은 뒤
센터링 transform 하나로 배치한다. 화면 크기가 달라져도 도형 상수는 손댈 필요가 없다.

Skia 캔버스 하나에 아래에서 위로 쌓는다:

1. 배경 그라디언트 — `LinearGradient`, 캔버스 전체
2. 하우징 — 키캡이 박힌 검은 타. **배경 위로 방향 있는 그림자를 드리운다**
   (`Box` + `BoxShadow`, 왼쪽 아래로, 고정)
3. 키캡 스커트 — 측면. 우물 안쪽에 작은 그림자를 떨어뜨리고 눌림에 따라 짧아진다
4. 키캡 상판 — `RoundedRect`

> **구현 중 정정.** 처음엔 그림자를 스커트에만 달았는데 **사실상 안 보였다** —
> 하우징(152pt)이 스커트(120pt)보다 커서 그림자가 전부 검은 하우징 위에 떨어졌다.
> Skia를 고른 근거가 방향 있는 그림자였으므로 큰 그림자를 하우징으로 옮겼다.

부수 효과: 탭 영역이 300pt 상자에서 **화면 전체**로 넓어진다. 피젯 토이에선 타깃이
큰 게 이득이라 그대로 둔다.
